...
import { CheckCircle2, HelpCircle, ListTodo, Plus } from "lucide-react";
...
export function DecisionDock() {
  ...
  // Prefer meeting-scoped actions if any; else all project actions
  ...
  return (
    <aside className="flex h-full flex-col border-l border-border bg-card">
      <div className="border-b border-border px-4 py-3">
        <h2 className="text-sm font-semibold tracking-tight">Decision Dock</h2>
        <p className="text-xs text-muted">Decisions stay visible while discussion scrolls</p>
      </div>
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-6">
        <section>
          ...
          {decisions.length === 0 ? (
            <p className="mt-2 text-xs text-muted-foreground">None yet — capture conclusions here</p>
          ) : (
            <ul className="mt-2 space-y-2">
              {decisions.map((d) => (
                ...
                >
                  <span className="text-support font-medium">✓</span> {d.title}
                  {d.whyBreakdown && d.whyBreakdown.length > 0 && (
                    <p className="mt-1 text-[11px] text-muted">
                      Why: {d.whyBreakdown.map((w) => w.personaName).join(", ")}
                    </p>
                  )}
                </li>
              ))}
        ...
        <section>
          ...
          {openQuestions.length === 0 ? (
            ...
            <ul className="mt-2 space-y-1.5">
              {openQuestions.map((q) => (
                <li key={q} className="text-sm text-foreground/90">
              ...
              {actions.map((a) => (
                <li key={a.id} className="flex items-start gap-2 text-sm">
                  <button
                    ...
                    className={
                      a.status === "done"
                        ? "text-support line-through text-muted"
                        : "text-foreground"
                    }
                  >
                    <span className="mr-1">{a.status === "done" ? "✓" : "→"}</span>
                    {a.title}
                  </button>
                </li>
              ))}
